...
let msSession = null; // { name, branch } — यही फोनमा लगइन गरेको कर्मचारी (जुन ब्राउजरमा index.html बाट पनि लगइन भइसकेको भए त्यही सत्र प्रयोग हुन्छ)

document.addEventListener("DOMContentLoaded", async () => {
  msSettings = await DB.getSettings();
  if (msSettings) {
    document.getElementById("msOffice").textContent = msSettings.officeName;
    document.getElementById("msLoginOffice").textContent = msSettings.officeName;
  ...
  msSession = rawSession ? JSON.parse(rawSession) : null;
  renderMsSessionSub();
  maybeShowMsLoginModal();
  bindMsLogin();
...
async function confirmPhotoReturn(fileId, returnPhotoThumb){
  ...
  await DB.updateFile(fileId, { status: "returned", returnedAt: `${bs} | ${ad}`, returnPhotoThumb, returnScannedBy: msSession ? msSession.name : null });
...
function renderMsSessionSub(){
  const el = document.getElementById("msSessionSub");
  el.textContent = msSession ? `👤 ${msSession.name} · ${msSession.branch}` : "मोबाइल QR स्क्यानर";
}

function maybeShowMsLoginModal(){
  const modal = document.getElementById("msLoginModal");
  if (msSession) { modal.classList.add("hidden"); return; }
  if (!msSettings || !msSettings.employees || msSettings.employees.length === 0) {
    modal.classList.add("hidden"); // कर्मचारी अझै नथपिएको भए लगइन बिना अगाडि बढ्न दिने
    return;
  }
  const sel = document.getElementById("msLoginEmployeeSelect");
  sel.innerHTML = msSettings.employees
    .map(e => `<option value="${escapeMs(e.name)}">${escapeMs(e.name)} — ${escapeMs(e.branch)}</option>`)
    .join("");
  modal.classList.remove("hidden");
}

function bindMsLogin(){
  document.getElementById("btnMsDoLogin").addEventListener("click", () => {
    const name = document.getElementById("msLoginEmployeeSelect").value;
    const pin = document.getElementById("msLoginPin").value.trim();
    const errEl = document.getElementById("msLoginError");
    const emp = (msSettings.employees || []).find(e => e.name === name);
    if (!emp) { errEl.textContent = "कर्मचारी छान्नुहोस्।"; errEl.classList.remove("hidden"); return; }
    if (emp.pin && emp.pin !== pin) { errEl.textContent = "PIN मिलेन।"; errEl.classList.remove("hidden"); return; }
    errEl.classList.add("hidden");
    msSession = { name: emp.name, branch: emp.branch };
    localStorage.setItem("fts_session", JSON.stringify(msSession));
    document.getElementById("msLoginModal").classList.add("hidden");
    renderMsSessionSub();
  });
}
...
async function handleScanResult(fileId){
  ...
  if (msMode === "taken") {
    ...
    await DB.updateFile(fileId, { status: "taken", takenAt: `${bs} | ${ad}`, takenScannedBy: msSession ? msSession.name : null });
    ...
    await DB.updateFile(fileId, { status: "returned", returnedAt: `${bs} | ${ad}`, returnScannedBy: msSession ? msSession.name : null });
